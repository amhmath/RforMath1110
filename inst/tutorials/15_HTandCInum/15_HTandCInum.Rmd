...
The variable `SentenceMonths` is a column within the `SDNYdrug` data frame. Remember that you can access columns in a data frame with the `$` operator. Remember, we compute the mean with the `mean()` function. `mean(SDNYdrug$SentenceMonths)`
</div>
...
Remember that you can compute the standard deviation in R with the `sd()` function. Additionally, you can find the number of rows in a data frame by passing the name of the data frame to R's `nrow()` function. The `sqrt()` function will also be useful to you. `sd(SDNYdrug$SentenceMonths)/sqrt(nrow(SDNYdrug))`
</div>
...
Did you try using 1.96? Remember that we can't use the normal distribution here. You can find the critical value you need by making use of the `qt()` function. `qt(.975,nrow(SDNYdrug)-1)`
</div>
...
mean(SDNYdrug$SentenceMonths)-qt(.975,nrow(SDNYdrug)-1)*sd(SDNYdrug$SentenceMonths)/sqrt(nrow(SDNYdrug))
#or
...
<div id = "sentencing-ht-9-hint"> Since `whiteSentences` and `nonWhiteSentences` are vectors of sentence lengths rather than full data frames, the `nrow()` function won't work on them. Try using the `length()` function instead. `length(whiteSentences)`</div>
...
```
<div id = "sentencing-ht-10-hint"> Again, since `whiteSentences` and `nonWhiteSentences` are vectors of sentence lengths rather than full data frames, the `nrow()` function won't work on them. Try using the `length()` function instead. `length(nonwhiteSentences)`</div>
...
```
```{r sentencing-ht-17-solution}
sqrt((sd(whiteSentences)^2/length(whiteSentences)) + (sd(nonWhiteSentences)^2/length(nonWhiteSentences)))
#or
...
```
```{r sentencing-ht-18-solution}
((mean(whiteSentences) - mean(nonWhiteSentences)) - 0)/((sqrt((sd(whiteSentences)^2/length(whiteSentences)) + (sd(nonWhiteSentences)^2/length(nonWhiteSentences)))))
#or
-14.942/6.994
...
quiz(
  ...
  answer("Sample data cannot be used to test a claim about a population -- different samples yield different results and so any claim about a population is nonsensical unless a census was conducted."),
  allow_retry = TRUE,
  random_answer_order = TRUE
  ),
  question_radio(
...
learnrhash::encoder_ui(
  ui_before = shiny::div(
    ...
    "solutions, please click the button below to generate, copy and paste your hash",
    "into the appropriate hash assignment in MyOpenMath.",
    shiny::tags$br()
  ), 
