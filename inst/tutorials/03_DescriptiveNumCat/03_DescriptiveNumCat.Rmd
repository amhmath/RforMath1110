...
1. Use the code block below as a calculator to compute the mean of `Sample_One`. (Add the carat sizes in column one and divide by 8.)
```{r meanSampleOne, exercise = TRUE}

```

```{r meanSampleOne-solution}
(.41+1.01+1.22+1.02+.30+.30+1.00+1.17)/8
```
...
question_checkbox(
  ...
  answer("Take samples of smaller sizes",message="Smaller samples sizes not a correct choice."),
  answer("Stratify the diamonds according to price (low, medium, and high) and take a random sample of diamonds of each price category",message="Stratifying not a correct choice."),
  allow_retry = TRUE,
  random_answer_order = FALSE
)
)
...
question_checkbox(
  ...
  answer("Worker A and Worker B had identical door-knocking strategies", message="A and B had same mean/median, but very different strategies."),
...
* Note that different methods and statistical software programs will find slightly different $Q_1$ and $Q_3$ values, which affects the interquartile range. These variations stem from alternate ways of finding percentiles. 

 * Percentile is a fairly common word. Surprisingly, there isn’t a single standard definition for it. Consequently, there are MANY methods for calculating percentiles. For example, the kth percentile might be defined in either of the two following ways:
    1) The smallest value that is greater than or equal to k percent of values. 
    2) An interpolated value between the two closest ranks.
    
  
Let us consider the following data set of 11 values:
  
$\{2,4,6,8,13,16,22,35,40,42,48\}$
  
  Using definition 1):

  * To find $Q_1$, we take 25% of 11, which is 2.75.  We then round up to 3.  The 25th percentile is *greater than or equal* to three values, so we pick the 3rd value, which is 6. 

  * To find $Q_3$, we take 75% of 11, which is 8.25.  We then round up to 9.  The 75th percentile is *greater than or equal* to nine values, so we pick the 9th value, which is 40. 

  * To find the *IQR* we find $Q_3-Q_1=40-6=34$. 

  Using definition 2): 

  * To find $Q_1$, we first calculate the rank for the 25th percentile, that is, rank = $p(n-1)+1$, where $p$ = the percentile and $n$ = the sample size. For our example, we find the rank$=0.25*(11 - 1)+1 = 3.5$. If the rank in step 1 is an integer, we find the data value that corresponds to that rank and use it for the percentile. If the rank is not an integer, we need to interpolate between the two closest observations. For our example, 3.5 means we have to find the "3-and-a-half$^{th}$" data point, which falls halfway between data points 6 and 8, which is 7. 
     
  * To find $Q_3$, we calculate the rank$=0.75*(10)+1=8.5$, so halfway between the 8th and 9th data points, i.e., 35 and 40, is 37.5.
 
   * To find the *IQR* we find $Q_3-Q_1=37.5-7=30.5$. 


  NOTE: R uses definition 2).
