...
ggplot(data=diamonds) + geom_point(mapping = aes(x = carat, y = price), color = "blue", alpha = .15) + labs(title = "Carat by Price", x = "Carat", y = "Price")
ggplot(data=diamonds) + geom_point(mapping = aes(x = carat, y = depth), color = "blue", alpha = .15) + labs(title = "Depth by Carat", x = "Depth", y = "Carat")
...
ggplot(data = pop) + geom_point(mapping = aes(x = x, y = y, color = samp), size = 2, alpha = 0.8) + scale_color_manual(values = c("blue", "orange")) + guides(<scale> = "none") + labs(title = "Simple Random Sample")
```
</center>
...
learnrhash::encoder_ui(
  ui_before = shiny::div(
    ...
    "enter it into the appropriate hash assignment in MyOpenMath." 
    shiny::tags$br()
  ), 
