...
question_checkbox(
  ...
  answer("$\\begin{array}{ll} H_0: & \\text{NH prison racial demographics match those of the State.}\\\\ H_a: & \\text{The distribution is different}\\end{array}$", correct = TRUE),
...
Use the code block below to compute the $p$-value associated with this test. Remember that you can use the function `pchisq(q, df)` to compute the probability to the LEFT of the boundary value `q` under a $\chi^2$ distribution with `df` degrees of freedom -- but we want the area to the RIGHT.

```{r compute-p, exercise = TRUE}

```
```{r compute-p-solution}
1-pchisq(69.15,3)
...
Okay, you've completed half of the two-way table. I'll fill in the rest in the *Expected Results* table below. The second table you see holds the actual *Observed Results*:
<div class = "row">
<div class = "md-col-5">
<center>
**Expected Results** <br/>
...
<div id="df-ind-hint">
**Hint:** For a $\chi^2$ test of independence we take the product between one less than the number of levels of each variable. That is, if variable one has $M$ levels and variable two has $P$ levels then the degrees of freedom is $\left(M-1\right)\left(P-1\right)$.
</div>
