...
+   In [**blue**]{style="color:blue"} is a normal distribution with $\mu = 0$ and $\sigma = 5$
+   In [**red**]{style="color:red"} is a normal distribution with $\mu = 0$ and $\sigma = 0.5$
...
The block below is preset to compute the Bob's percentile. Execute the code cell and then adapt the code to find Sally's percentile. Use your results to answer the questions below. Recall: Bob scored a 1400 on the SAT (mean=1068, sd=210) while Sally scored a 31 on the the ACT (mean=20.8, sd=5.8.)
...
grade_result(
  pass_if(~ (abs(.result - pnorm(1.49, 0, 1)) < 0.001))
)
```
:::
...
ggplot(NULL, aes(c(-3, 3))) +
  geom_area(stat = "function", fun = dnorm, args = list(mean = 0, sd = 1), xlim = c(-3, 1.49), fill = "steelblue", alpha = 0.5) + geom_line(mapping = aes(x = seq(-3, 3, length.out = 200), y = dnorm(seq(-3, 3, length.out = 200), 0, 1)), color = "blue", lwd = 1.25) + xlab("") + ylab("") + ggtitle("") + xlim(c(-3, 3)) + scale_x_continuous(breaks = c(0, 1.49)) + theme(axis.text.x = element_text(size = 12, angle = 45))
```
:::
:::
...
```
```{r pnorm-ex2-left-solution}
pnorm(-1.83,0,1)
```

```{r pnorm-ex2-left-check}
grade_result(
  pass_if(~ (abs(.result - pnorm(-1.83, 0, 1)) < 0.001))
)
```
:::
...
ggplot(NULL, aes(c(-3, 3))) +
  geom_area(stat = "function", fun = dnorm, args = list(mean = 0, sd = 1), xlim = c(-3, -1.83), fill = "steelblue", alpha = 0.5) + geom_line(mapping = aes(x = seq(-3, 3, length.out = 200), y = dnorm(seq(-3, 3, length.out = 200), 0, 1)), color = "blue", lwd = 1.25) + xlab("") + ylab("") + ggtitle("") + xlim(c(-3, 3)) + scale_x_continuous(breaks = c(0, -1.83)) + theme(axis.text.x = element_text(size = 12, angle = 45))
```
:::
:::
...
```
```{r pnorm-ex3-left-solution}
1-pnorm(-2.17,0,1)
```

```{r pnorm-ex3-left-check}
grade_result(
  pass_if(~ (abs(.result - (1 - pnorm(-2.17, 0, 1))) < 0.001))
)
```
:::
...
ggplot(NULL, aes(c(-3, 3))) +
  geom_area(stat = "function", fun = dnorm, args = list(mean = 0, sd = 1), xlim = c(-2.17, 3), fill = "steelblue", alpha = 0.5) + geom_line(mapping = aes(x = seq(-3, 3, length.out = 200), y = dnorm(seq(-3, 3, length.out = 200), 0, 1)), color = "blue", lwd = 1.25) + xlab("") + ylab("") + ggtitle("") + xlim(c(-3, 3)) + scale_x_continuous(breaks = c(0, -2.17)) + theme(axis.text.x = element_text(size = 12, angle = 45))
```
:::
:::
...
```
```{r pnorm-ex4-left-solution}
1-pnorm(2.12,0,1)
```

```{r pnorm-ex4-left-check}
grade_result(
  pass_if(~ (abs(.result - (1 - pnorm(2.12, 0, 1))) < 0.001))
)
```
:::
...
ggplot(NULL, aes(c(-3, 3))) +
  geom_area(stat = "function", fun = dnorm, args = list(mean = 0, sd = 1), xlim = c(2.12, 3), fill = "steelblue", alpha = 0.5) + geom_line(mapping = aes(x = seq(-3, 3, length.out = 200), y = dnorm(seq(-3, 3, length.out = 200), 0, 1)), color = "blue", lwd = 1.25) + xlab("") + ylab("") + ggtitle("") + xlim(c(-3, 3)) + scale_x_continuous(breaks = c(0, 2.12)) + theme(axis.text.x = element_text(size = 12, angle = 45))
```
:::
:::
...
```
```{r pnorm-ex5-left-solution}
pnorm(2.08, 0, 1) - pnorm(-1.04, 0, 1)
```

```{r pnorm-ex5-left-check}
grade_result(
  pass_if(~ (abs(.result - (pnorm(2.08, 0, 1) - pnorm(-1.04, 0, 1))) < 0.001))
)
```
:::
...
ggplot(NULL, aes(c(-3, 3))) +
  geom_area(stat = "function", fun = dnorm, args = list(mean = 0, sd = 1), xlim = c(-1.04, 2.08), fill = "steelblue", alpha = 0.5) + geom_line(mapping = aes(x = seq(-3, 3, length.out = 200), y = dnorm(seq(-3, 3, length.out = 200), 0, 1)), color = "blue", lwd = 1.25) + xlab("") + ylab("") + ggtitle("") + xlim(c(-3, 3)) + scale_x_continuous(breaks = c(-1.04, 0, 2.08)) + theme(axis.text.x = element_text(size = 12, angle = 45))
```
:::
:::
...
```
```{r pnorm-ex6-left-solution}
pnorm(-1, 0, 1) + (1- pnorm(1.7, 0, 1))
```

```{r pnorm-ex6-left-check}
grade_result(
  pass_if(~ (abs(.result - (pnorm(-1, 0, 1) +(1 - pnorm(1.7, 0, 1)))) < 0.001))
)
```
:::
...
ggplot(NULL, aes(c(-3, 3))) +
  geom_area(stat = "function", fun = dnorm, args = list(mean = 0, sd = 1), xlim = c(-3, -1), fill = "steelblue", alpha = 0.5) + geom_area(stat = "function", fun = dnorm, args = list(mean = 0, sd = 1), xlim = c(1.7, 3), fill = "steelblue", alpha = 0.5) + geom_line(mapping = aes(x = seq(-3, 3, length.out = 200), y = dnorm(seq(-3, 3, length.out = 200), 0, 1)), color = "blue", lwd = 1.25) + xlab("") + ylab("") + ggtitle("") + xlim(c(-3, 3)) + scale_x_continuous(breaks = c(-1, 0, 1.7)) + theme(axis.text.x = element_text(size = 12, angle = 45))
```
:::
:::
...
```
```{r pnorm-ex7-left-solution}
2*pnorm(-2.03, 0, 1)
```

```{r pnorm-ex7-left-check}
grade_result(
  pass_if(~ (abs(.result - (pnorm(-2.03, 0, 1) +(1 - pnorm(2.03, 0, 1)))) < 0.001))
)
```
:::
...
ggplot(NULL, aes(c(-3, 3))) +
  geom_area(stat = "function", fun = dnorm, args = list(mean = 0, sd = 1), xlim = c(-3, -2.03), fill = "steelblue", alpha = 0.5) + geom_area(stat = "function", fun = dnorm, args = list(mean = 0, sd = 1), xlim = c(2.03, 3), fill = "steelblue", alpha = 0.5) + geom_line(mapping = aes(x = seq(-3, 3, length.out = 200), y = dnorm(seq(-3, 3, length.out = 200), 0, 1)), color = "blue", lwd = 1.25) + xlab("") + ylab("") + ggtitle("") + xlim(c(-3, 3)) + scale_x_continuous(breaks = c(-2.03, 0, 2.03)) + theme(axis.text.x = element_text(size = 12, angle = 45))
```
:::
:::
...
```
```{r chick-pox-sd-solution}
sqrt(68*0.9*0.1)
```

```{r chick-pox-sd-check}
grade_result(
  pass_if(~ (abs(.result - (sqrt(68*0.9*0.1))) < 0.1))
)
```

```{r surprised-if, echo = FALSE}
question_checkbox(
  paste0("Would you be surprised if your sample contained at most ", floor((68*0.9) - (3.5*sqrt(68*0.9*0.1))), " adults who have had chicken pox? Why? (select all that are appropriate)"),
...
```
```{r chick-pox-prob-exact1-solution}
dbinom(61,68,0.9)
```

```{r chick-pox-prob-exact1-check}
grade_result(
  pass_if(~ (abs(.result - (dbinom(floor(0.9*68), 68, 0.9))) < 0.001))
)
```

+   Find the probability that exactly 7 adults in your sample of size 68 will *not* have had chickenpox.

```{r chick-pox-prob-exact2, exercise = TRUE}

```
```{r chick-pox-prob-exact2-solution}
dbinom(7,68,0.1)
```

```{r chick-pox-prob-exact2-check}
grade_result(
  pass_if(~ (abs(.result - (dbinom(floor(0.11*68), 68, 0.1))) < 0.001))
)
```
...
```
```{r chick-pox-prob-atmost-solution}
pbinom(51,68,0.9)
```

```{r chick-pox-prob-atmost-check}
grade_result(
  pass_if(~ (abs(.result - (pbinom(floor(0.75*68), 68, 0.9))) < 0.00005))
)
```
...
```
```{r chick-pox-prob-atleast-solution}
1-pbinom(63,68,0.9)
```

```{r chick-pox-prob-atleast-check}
grade_result(
  pass_if(~ (abs(.result - ( 1 - pbinom((floor(0.95*68) - 1), 68, 0.9))) < 0.0001))
)
```
...
```
```{r chick-pox-prob-between-solution}
pbinom(60,68,0.9)-pbinom(54,68,0.9)
```

```{r chick-pox-prob-between-check}
grade_result(
  pass_if(~ (abs(.result - ( pbinom((floor(0.9*68) - 1), 68, 0.9) - pbinom(floor(0.8*68), 68, 0.9))) < 0.0001))
)
```
...
```
```{r GRE-VR-percent-above-solution}
1-pnorm(160, 151, 7)
```
...
learnrhash::encoder_ui(
  ui_before = shiny::div(
    ...
    "enter it in the appropriate Hash assignment in MyOpenMath.",
    shiny::tags$br()
  ), 
  ui_after = shiny::tags$a(href = "https://myopenmath.com", "MyOpenMath")
)
```
