...
# There are two (2) ways to find a right tail:
1-pnorm(boundary3,0,1); pnorm(boundary3, 0, 1, lower.tail=F)
...
1-pnorm(boundary4, 0, 1) ; pnorm(boundary4, 0, 1, lower.tail=F)
```
...
pnorm(boundary6a, 0, 1)+ (1-pnorm(boundary6b,0,1); pnorm(boundary6a, 0, 1)+ pnorm(boundary6b,0,1,lower.tail=F)   
```
...
dbinom(floor(0.9*chickPoxSampSize), chickPoxSampSize, 0.9)
```
...
dbinom(floor(0.1*chickPoxSampSize), chickPoxSampSize, 0.1)
```
...
pbinom(floor(0.75*chickPoxSampSize), chickPoxSampSize, 0.9)
```
...
1 - pbinom((floor(0.95*chickPoxSampSize) - 1), chickPoxSampSize, 0.9)
```
...
pbinom((floor(0.9*chickPoxSampSize) - 1), chickPoxSampSize, 0.9) - pbinom(floor(0.8*chickPoxSampSize), chickPoxSampSize, 0.9)
```                                                                                                              
...
+ Find the proportion of test takers that Sophia *scored higher than* on the Quantitative Reasoning exam (that is, find her percentile).
...
1-pnorm(160, 151, 7); pnorm(160,151,7,lower.tail=F)
``` 
...
learnrhash::encoder_ui(
  ui_before = shiny::div(
    ...
    "enter it on this week's Google Form in Blackboard.",
    shiny::tags$br()
  ), 
  ui_after = shiny::tags$a(href = "https://blackboard.mville.edu", "Blackboard")
)
```
