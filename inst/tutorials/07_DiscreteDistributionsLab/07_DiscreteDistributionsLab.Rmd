...
To verify this, execute the command `kobe$basket[1:9]` using the code block below. Remember that this accesses the `basket` column of the `kobe` data frame, and the `[1:9]` says that we want entry 1 through entry 9 of that column.
...
ggplot() + geom_bar(aes(kobe_streak))
```
...
Since there are only two elements in the *outcomes*, the probability that we "flip" a coin and it lands heads is 0.5. Say we're trying to simulate an unfair coin that we know only lands heads 20% of the time. We can adjust for this by adding an argument called `prob`, which provides a vector of two probability weights.  (if we don't stipulate a probability, R assumes 50%-50%.) Execute the following code to simulate this unfair coin.
...
To make a valid comparison between Kobe and our simulated independent shooter, we need to align both their shooting percentage and the number of attempted shots. Use the code block below to help you find the number of shots we need to simulate. Hint: use `nrow()`.
...
```
<div id="discuss-q1-hint">
Use `ggplot()+geom_bar(aes(sim_streak))` to give you a better picture of the results.  Also, this question is asking for a discussion, that is, a sentence or two, so be sure to put your answer in quotes so R knows you are entering a string and not code.
</div>
...
```
<div id="discuss-q2-hint">
Use `sim_basket2 <- sample(c("H", "M"), size = 133, prob = c(0.45, 0.55), replace = TRUE)` and `sim_streak2<-calc_streak(sim-basket2)` to generate a second simulation.  Then run `ggplot()+geom_bar(aes(sim_streak2))` to give you a nice picture of the results.  Again, be sure to put your answers in quotes so R knows you are entering a string and not code.
</div>
