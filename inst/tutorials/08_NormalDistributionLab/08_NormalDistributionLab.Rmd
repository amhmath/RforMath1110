...
```
```{r plot-hgt-hists-solution}
ggplot()+geom_bar(aes(mdims$hgt))
ggplot()+geom_bar(aes(fdims$hgt))
...
**Question 1:** What is the probability that a randomly chosen young adult female is taller than 6 feet (about 182 cm)? Use the code block below to answer this question assuming that female heights are normally distributed. That is, use the `pnorm()` function we were introduced to in the previous workbook, and recall we used `fhgtmean` and `fhgtsd` for the mean and sd of female height. (The study that published this data set is clear to point out that the sample was not random and therefore inference to a general population is not suggested.  We do so here only as an exercise.)
```{r prob-height-norm, exercise = TRUE}

```
```{r prob-height-norm-solution}
...
grade_result(
  pass_if(~ (abs(.result - (1 - pnorm(182, 164.87, 6.54))) < 0.0005))
)
```
...
**Question 2:** What is the probability that a randomly chosen young adult female weighs more than 80kg? Use the code block below to answer this question making the assumption that female weights are normally distributed (recall that we said this assumption was not a good one). Also, we have assigned `fwgtmean` and `fwgtsd` to the mean and sd of female weight. 
```{r wgt-norm-prob, exercise = TRUE}

```
```{r wgt-norm-prob-solution}
1-pnorm(80,fwgtmean,fwgtsd)

```
```{r wgt-norm-prob-check}
grade_result(
  pass_if(~ (abs(.result - (1 - pnorm(80, 60.6, 9.62))) < 0.0005))
)
```
...
```
```{r wgt-emp-prob-solution}
sum(fdims$wgt > 80) / length(fdims$wgt)

```
```{r wgt-emp-prob-check}
grade_result(
  pass_if(~ (abs(.result - 0.04230769) < 0.0005))
)
```
