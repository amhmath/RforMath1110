...
question_checkbox(
  ...
  answer("A histogram",message="A histogram requires at least one numerical variable."),
  answer("A barplot with fill color", correct = TRUE),
  answer("A heatmap", correct = TRUE),
  answer("A scatterplot",message="A scatterplot  uses numerical data."),
  answer("Side-by-side boxplots", message="A boxplots requires at least one numerical variable."),
  allow_retry = TRUE
)
```
...
**Some Basic Exploratory Functions:** It is useful to know more about your dataset when you first start working with data. R has a few exploratory functions which you should know about: `names(mpg)` prints out a list of names of your data frame's columns, `head(mpg)` will show you the first six rows of the `mpg` dataset, `head(mpg,n)` will show you the first $n$ rows of the `mpg` dataset, `dim(mpg)` will show you the number of rows and columns in the dataset, and `glimpse(mpg)` will show you information about how R is treating the columns -- `<int>` and `<num>` denote numerical values while `<chr>` and `<fct>` represent character strings and categorical variables respectively. (Note, `glimpse(head(mpg,0))` gives you no data, but the output is a *cleaner* list of the variable information.) Use the code block below to try each of these functions and use the output to answer the questions that follow.
...
quiz(
  question_radio(
    ...
    answer("1", message="Remember, `dim(mpg)` will show you the number of rows and columns in the dataset."),
    answer("11", correct = TRUE),
    answer("234", message="This is the number of rows -- you want the numer of columns in the dataset."),
    answer("245", message="Remember, `dim(mpg)` will show you the number of rows and columns in the dataset."),
    answer("2574", message="Remember, `dim(mpg)` will show you the number of rows and columns in the dataset."),
    allow_retry = TRUE
  ),
  ...
  question_radio(
    ...
    answer("1", message="Remember, `dim(mpg)` will show you the number of rows and columns in the dataset."),
    answer("11", message="This is the number of columns -- you want the numer of rows in the dataset."),
  ...
  question_checkbox(
    "Which of the variables is R interpreting as numerical? Hint: use `glimpse(head(mpg,0))`",
    answer("manufacturer", message="manufacturer is not numerical."),
    answer("model", message="model is not numerical."),
    answer("displ", correct = TRUE),
    answer("cyl", correct = TRUE),
    answer("trans", message="trans is not numerical."),
    answer("drv", message="drv is not numerical."),
    answer("cty", correct = TRUE),
    answer("hwy", correct = TRUE),
    answer("fl", message="fl is not numerical."),
    answer("class", message="class is not numerical."),
    allow_retry = TRUE
  )
)
```
...
```{r explore-diamonds-solution}
dim(diamonds)
glimpse(head(diamonds,0))
```

Were you able to obtain the information you were looking for? Be sure to ask a question if not!

Note that `<ord>` indicates ordered categorical variables, and `<dbl>` indicates numerical variables with two decimal places.
...
quiz(
  question_radio(
    ...
    answer("It is impossible to tell whether the variables are associated or independent"),
...
ggplot(data = mpg) + geom_point(mapping = aes(x = cyl, y = hwy, group = cyl))
```
...
quiz(
  ...
  question_checkbox(
    ...
    answer("A histogram", message="A histogram requires at least one numerical variable."),
    answer("A side-by-side boxplot", message="A boxplot requires at least one numerical variable."),
...
We've seen plots for categorical variables earlier in this workbook. Let's revisit a barplot with fill, similar to the one we constructed between `class` and `drv` earlier and check two alternative plots which give different insights. We'll explore the transmission and drive variables.

The following code stores a more usable version of the `trans` variable as `trans2`. It removes the last 5 characters in the label, leaving just "auto" or "manual".  For example, "auto (14)" becomes "auto".

```{r engineer-trans2, echo = TRUE, eval = TRUE}
mpg$trans2 <- str_sub(mpg$trans, 1, -5)
```

We'll use this for the next exercise.
...
mpg$trans2 <- str_sub(mpg$trans, 1, -5)
ggplot(data = mpg) + geom_bar(mapping = aes(x = trans2, fill = drv))
```

```{r trans-drv-plot-solution}
mpg$trans2 <- str_sub(mpg$trans, 1, -5)
ggplot(data = mpg) + geom_bar(mapping = aes(x = trans2, fill = drv), position = "fill")
```
...
mosaicplot(mpg$trans2 ~ mpg$drv, main = "Transmission and Drive Types",xlab="trans",ylab="drive")
```
...
learnrhash::encoder_ui(
  ...
  ui_after = shiny::tags$a(href = "https://www.myopenmath.com", "MyOpenMath")
)
```
